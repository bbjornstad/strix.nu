...
export def remove [
    ...remove: string@'completes raur remove target'
    --nofuzz (-f)
] {
    let inp = (
        if ($remove | is-empty) {
    ...
    let remove_targets = (
        if not $nofuzz {
            $inp | ^fzf --multi --ansi --preview 'paru -Qi {1}'
        } else {
            $inp
        }
    )
...
export def --wrapped search [
    ...
    let target_mode = (
        ["Search(AUR)" "Install" "Query(local)"]
